...
class NCManager:
    ...
    def apply(self, config: NCConfig) -> bool:
        ...
        if _native_tools_available():
            if self._apply_native(config, rnnoise_plugin):
                ...
                # (this resets output_source, so re-assert it after).
                self._teardown_chain()
    ...
    def _apply_legacy(self, config: NCConfig, rnnoise_plugin: str) -> bool:
        ...
        ok = self._load_ladspa_source(pulse, name, current,
                                      rnnoise_plugin, RNNOISE_LABEL, RNNOISE_CONTROLS, 'RNNoise')
        if not ok:
    ...
    def _native_stages(self, config: NCConfig,
        ...
        # Same tuning as RNNOISE_CONTROLS on the legacy path (VAD threshold %,
        # grace ms, retroactive grace ms) — keeps quiet consonants intact for
        # the voice changer downstream.
        vad, grace, retro = (float(v) for v in RNNOISE_CONTROLS.split(','))
        stages.append(('rnnoise', 'ladspa', rnnoise_plugin, RNNOISE_LABEL, {
            'VAD Threshold (%)': vad,
            'VAD Grace Period (ms)': grace,
            'Retroactive VAD Grace (ms)': retro,
        }))
