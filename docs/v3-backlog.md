...
- [ ] **[E10] AI voice changer port to Rust** *(stretch)*
  ...
  - [ ] [E10-S5b] Cut GUI (`vc_widget.py`, `vc_calibration_wizard.py`, `dbus_wrapper.py`) over from the legacy Python daemon's VC service to the Rust engine's `VcInterface`
  - [ ] [E10-S6a] `vc/inference/` — unified `ort`-based engine (ContentVec, RMVPE, synthesizer) replacing the separate PyTorch/OpenVINO backends, execution-provider selection (`providers.rs`), brute-force k-NN retrieval (`retrieval.rs`) replacing the `libfaiss` dependency
...
- **[E10-S5b] Cut GUI over to `VcInterface`**
  Update `vc_widget.py`, `vc_calibration_wizard.py`, and `gui/dbus_wrapper.py` to call the Rust engine's `VcInterface` instead of the legacy Python daemon's `ArctisManagerDbusVCService`. The LADSPA tab, model management, HuggingFace search/download, and calibration recording map onto real methods; the RVC mode selector and calibration's "render" step must be hidden/disabled based on `GetVCCapabilities`'s `rvc.available: false`, matching the existing pattern `vc_widget.py` already uses to hide unavailable LADSPA effects. Not done today because it needs to be verified against a live GUI session, which isn't possible in this environment.

- **[E10-S6a] `vc/inference/` — unified `ort` engine**
  Single Rust inference module (ContentVec → RMVPE f0 → retrieval blend → synthesizer) via the `ort` crate, replacing the separate `pytorch_impl.py`/`openvino_impl.py` backends with one execution-provider selection (`providers.rs`: CUDA/ROCm/OpenVINO/CPU). Brute-force weighted k-NN retrieval (`retrieval.rs`) over the model's `.index` feature vectors replaces the `libfaiss` dependency. Model conversion (`.pth` → ONNX) stays a one-shot offline Python script, not a daemon runtime dependency. This engine serves both the live VC chain (via [E10-S5a]/[E10-S5b]) and calibration rendering ([E10-S6b]).
