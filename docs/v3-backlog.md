...
- [ ] **[E10] AI voice changer port to Rust** *(stretch)*
  ...
  - [ ] [E10-S6b] `render_start`/`render_blocking` — the render engine itself is **done and live-verified** (fresh `Pipeline` per candidate over the recording at the live chain's hop cadence, writes `variant_<label>.wav`, transitions `Recorded → Rendering → Done`); still open: D-Bus wiring (`CalibrationStartRender` doesn't call it yet), the persisted RVC settings it would read from (no Rust equivalent of Python's `VCSettings` exists), and generalising the GUI wizard for the new pitch pre-scan stage (`propose_pitch_variants`, added this round — a bass-baritone-vs-soprano/contralto register mismatch dominated perceived quality far more than any dynamics parameter in live listening).
  ...
  - [ ] [E10-S8] Wire the *live* VC chain to the [E10-S6a] engine — distinct from [E10-S6b] (calibration render, offline, one recording in/out): this is the real-time path a live call actually uses. `GetRVCMetrics`/`SetRVCLiveParams` aren't in `VcInterface` yet (would be fake stubs without this); needs real mic-in/speaker-out audio I/O wired through `mic_router.rs`, model loading orchestration, and the same persisted RVC settings [E10-S6b] also needs (model selection, pitch, tuning — no Rust `VCSettings` equivalent exists yet). Blocked on the same `model_sr`-from-`.onnx` gap noted under [E10-S6b] above. Not started.

**Outstanding manual verification**: none of this session's work (cuDNN self-heal, the [E10-S6b] render engine, the pitch pre-scan, `normalize_input_level`) has been exercised through the *real* running daemon + GUI yet — every check so far is the offline `#[ignore]`d Rust test harnesses. Needs `sudo make install && systemctl --user restart lam-daemon.service` plus a real pass through the GUI once [E10-S8]/the rest of [E10-S6b] is wired up enough to reach it that way (today the render engine is only reachable via the Rust test harness, not any D-Bus method a running GUI could hit).
...
- **[E10-S6b] Wire calibration rendering onto the engine** — render engine done and live-verified; D-Bus/settings/GUI wiring is the remaining piece (see below).
  ...
  **Known open gap, blocks this and the eventual live chain equally:** nothing in this engine can determine a synthesizer's native sample rate from its `.onnx` file — `SynthSession::load` reads `inter_channels`/`t_audio` from static input shapes, but sample rate isn't recoverable from those alone. The Python reference reads it from the original `.pth` checkpoint's `sr`/`config[-1]` field, which this Rust daemon has no path to (no PyTorch pickle reading by design). `render_start`'s `RenderModel::sample_rate` is therefore an explicit required parameter for now. Clean fix, not done here: have `export_onnx.py` write it into the exported `.onnx`'s `metadata_props`, read back via `ort`'s `Session::metadata()`.

  **Also not done here:** D-Bus wiring (`CalibrationStartRender` in `dbus.rs` still only exercises record_start/record_stop) and the persisted RVC settings it would read from (model selection, pitch, tuning — no equivalent of Python's `VCSettings` exists in the Rust daemon yet; `VcLadspaConfig` is LADSPA-only). The GUI wizard (`vc_calibration_wizard.py`) also still assumes exactly 3 dynamics-only variants and would need generalising for a variable-length, two-stage (pitch then dynamics) flow.
