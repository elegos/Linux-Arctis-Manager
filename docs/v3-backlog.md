...
- **[E10-S6a] `vc/inference/` — unified `ort` engine** — all three ONNX sessions (ContentVec, RMVPE, synthesizer) load and run, live-verified against real `onnxruntime` output; the streaming window state machine is the one remaining piece.
  ...
  Still missing: the streaming window state machine that ties all three sessions + the DSP/retrieval modules together into the real-time chain — the last piece of [E10-S6a].
