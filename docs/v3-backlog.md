...
- [ ] **[E10] AI voice changer port to Rust** *(stretch)*
  ...
  - [ ] [E10-S4] `vc_calibration.rs` — guided calibration state machine (`pw-record` subprocess), same lifecycle as the Python `CalibrationSession`
    > Blocked: the recording half (`record_start`/`record_stop`, downmix, `propose_variants`) is done. `render_start`/`_render` need the RVC inference pipeline ([E10-S6]) to convert the recording through candidate params — not implemented until then.
