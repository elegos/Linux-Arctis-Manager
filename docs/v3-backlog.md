...
- **[E10-S7] Guided `libonnxruntime` install helper**
  ...
  **Design** (agreed, not yet implemented):
  ...
  6. **Never auto-install**: the daemon/GUI only ever *shows* the instructions and a copy button; it does not invoke `pkexec`/`sudo` itself. Matches the project's existing house style (guided wizards, manual HF token entry) over silent/automatic system changes, and sidesteps the privilege-escalation trust question entirely.
