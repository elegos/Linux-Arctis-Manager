...
impl HidTransport {
    ...
    pub async fn write_interrupt(&mut self, data: &[u8]) -> std::io::Result<()> {
    ...
    pub async fn read_interrupt(&mut self, timeout: Duration) -> Result<Vec<u8>, ReadError> {
        ...
        match tokio::time::timeout(timeout, self.file.read(&mut buf)).await {
            Ok(Ok(n)) => {
                buf.truncate(n);
                Ok(buf)
            }
    ...
    pub fn write_feature(&self, data: &[u8]) -> std::io::Result<()> {
        // HIDIOCSFEATURE reads from user memory; we copy to satisfy the &mut
        // requirement of the nix macro while keeping the public API immutable.
        let mut buf = data.to_vec();
        unsafe { hid_set_feature(self.file.as_raw_fd(), &mut buf) }
            .map(|_| ())
    ...
    pub fn read_feature(&self, buf: &mut [u8]) -> std::io::Result<usize> {
        unsafe { hid_get_feature(self.file.as_raw_fd(), buf) }
            .map(|n| n as usize)
            .map_err(|e| std::io::Error::from_raw_os_error(e as i32))
    }
}
...
mod tests {
    ...
    fn make_pair() -> (HidTransport, OwnedFd) {
        ...
        .expect("socketpair");
        (HidTransport::from_fd(engine_fd), peer_fd)
    }
    ...
    async fn write_then_read_roundtrip() {
        ...
        let mut t2 = HidTransport::from_fd(peer_fd);
