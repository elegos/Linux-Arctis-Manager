...
- v3: `GetEQCapabilities` now inspects the connected device's `apis` map: if `custom_eq` is present (Nova Pro family), returns `has_hw_eq: true, hw_band_mode: "fixed_10"` instead of always `false`.
...
- v3: Fixed `set-default-sink Arctis_Media` being called before the sink was created on connect; virtual sinks are now set up before the redirect.
...
- v3: Fixed duplicate loopback accumulation on daemon restart. `setup_sinks` previously relied on `pactl list modules short` args to detect an existing loopback, but PipeWire does not always preserve module args across client connections, causing the check to silently fail and a new `module-loopback` to be loaded on every restart. After 10 restarts, the Arctis_Media signal was summed 10×, producing heavy perceived distortion. The fix: `sweep_loopbacks_for_source` now always unloads all existing loopbacks for `Arctis_Media.monitor` and `Arctis_Chat.monitor` at startup before creating exactly one new loopback each. The null-sink is still reused when present (checked via `pactl list sinks`, which is reliable).
...
- Fixed zombie GUI process on window close: the D-Bus settings signal thread (`_register_settings_dbus_signal`) was started as a non-daemon thread with a local `asyncio.Future` that was never resolved on shutdown. The future is now stored as `_stop_settings_signal_future` on `DbusWrapper` and resolved in `stop()`, allowing the process to exit cleanly.
