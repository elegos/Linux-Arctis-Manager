...
- CI now enforces Python code quality alongside the existing Rust checks: `ruff` (lint + import sorting, replacing the old `isort`/`autoflake` dev deps) and `basedpyright` (type checking) both run blocking in a new `python-ci.yaml` workflow, alongside a coverage floor (`pytest-cov`, currently 40%, set a few points below the measured baseline as a regression guard). `rust-ci.yaml` gained a `cargo-deny` step (licenses/bans/sources; advisories stay in `sast.yaml`'s existing non-blocking `cargo-audit` job so the two don't duplicate). Fixing the codebase to actually pass these surfaced two real bugs: `JsonSerializable.to_dict()` never actually excluded callable-valued fields (compared `type(x) != callable`, which can never be true), and `status_parser_fn.py`'s `percentage()` had a no-op self-assignment (`perc_min, perc_max = perc_min, perc_max`) left over from a since-abandoned change.
