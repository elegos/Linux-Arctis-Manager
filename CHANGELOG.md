...
- Rust daemon (v3): Arctis GameBuds (+ X) device support, ported from the raw vendor spec rather than the open, AI-written PR #57 — that PR was found to swap the `0x27`/`0xC5` command bytes between `volume_limiter`/`wear_sense`, model a 3-position `transparency` control as a continuous slider, and claim a listen interface that contradicts the raw spec's own declared value; none of its data was reused. Ships mic volume/sidetone/mute status, ANC/transparency (with the firmware's own level quantization), volume limiter, wear-sense, inactivity timer, RF/BT dual headset volume, 2.4G/BT audio-mode switching, battery/connection sync per earbud, and two independent 10-band parametric EQ profiles (one for the 2.4G RF link, one for Bluetooth) — almost everything live-pushed via the dongle's own sync interface, not just read at connect. The charging case ships as its own device (own PID, own sync interface, same "genuinely separate hardware" pattern as GameDAC vs. its headset) — per-earbud and case battery/charging status, lid open/closed. Button remapping is out of scope: real 8-gesture-slot feature, but no capability tag or UI exists for it yet. New engine builtin (`builtin:gamebuds_eq_bands`) for this device's band-write message, whose header omits the `connection_type` byte the existing generic parametric-EQ-bands builtin assumes.
...
- CI now enforces Python code quality alongside the existing Rust checks: `ruff` (lint + import sorting, replacing the old `isort`/`autoflake` dev deps) and `basedpyright` (type checking) both run blocking in a new `python-ci.yaml` workflow, alongside a coverage floor (`pytest-cov`). `rust-ci.yaml` gained a `cargo-deny` step (licenses/bans/sources; advisories stay in `sast.yaml`'s existing non-blocking `cargo-audit` job so the two don't duplicate). Fixing the codebase to actually pass these surfaced two real bugs: `JsonSerializable.to_dict()` never actually excluded callable-valued fields (compared `type(x) != callable`, which can never be true), and `status_parser_fn.py`'s `percentage()` had a no-op self-assignment (`perc_min, perc_max = perc_min, perc_max`) left over from a since-abandoned change.
- Added tests for every previously-untested non-GUI Python module (`eq_preset`, `i18n`, `utils`, `systemd`, the `lam-cli`/`lam-gui` entrypoints' testable helpers, and the voice-changer `RVCBackend`/`RVCParams` contract), raising overall coverage from 43% to 91% and the CI floor from 40% to 88%. `scripts/gui.py`'s `main()` (real `QApplication`/event loop bootstrap) is left untested — not practically unit-testable headlessly — and is the only remaining low-coverage file.

### Removed

- The old Python daemon (`core.py`, `dbus_service.py`, `scripts/daemon.py`, and everything only reachable from them — `eq_manager.py`, `mic_router.py`, `nc_manager.py`, `pactl.py`, `usb_devices_monitor.py`, `settings.py`, `app_matcher.py`, `steam_library.py`, `ai_deps.py`, the old `voice_changer/manager.py` and `voice_changer/settings.py`, the LADSPA chain, and every RVC module but the ONNX export path) has been deleted outright — it was leftover code from before the Rust daemon rewrite, already unreachable from `lam-gui`. `lam-cli` is trimmed to just its `tools arctis-devices` USB/HID introspection subcommand (still used for device porting, see `docs/device_support.md`); its udev-rules, desktop-entry, and setup-orchestration code is gone too, matching what the packaging changelogs already claimed ("lam-cli removed") but the source tree hadn't caught up to yet. The `lam-daemon` Python script entry point is gone from `pyproject.toml`. A handful of `constants.py` entries that only existed for this dead code (`UDEV_RULES_PATHS`, `SETTINGS_FOLDER`, `ARCTIS_VC_SINK`, `STEELSERIES_VENDOR_ID`, the D-Bus `Config` interface/path, the Pulse media/chat node names) are gone too, having no other reader.
