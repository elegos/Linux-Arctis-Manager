...
- Rust daemon (v3): Arctis Nova Elite device support (standard + "sng" SKU) — ANC (off/transparent/on, each with its own level), independent boom-mic and on-ear-mic sidetone, mic volume/mute-LED brightness, Bluetooth defaults, station display (home screen, screensaver, brightness), battery/connection/charging sync, hardware ChatMix. EQ (all domains) and the bulk mic/sidetone/line-out startup snapshot are not supported on this device yet — the device's EQ has no simple preset-select command at all, and its bulk-settings read exceeds the daemon's current HID feature-report size limit. Both tracked as a follow-up story, not shipped in this pass.
